...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "statsd://dokku-graphite-l:8125"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "statsd://dokku-graphite-test-with-underscores:8125"
...
@test "($PLUGIN_COMMAND_PREFIX:info) success with flag" {
  ...
  assert_output "statsd://dokku-graphite-l:8125"
